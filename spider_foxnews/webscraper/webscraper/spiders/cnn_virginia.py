...
first_names = ["Nicholas",  "E. W.",    "Tim",    "Corey",    "Matt"]
last_names =  ["Freitas",   "Jackson",  "Kaine",  "Stewart",  "Waters"]
...
class QuotesSpider(scrapy.Spider):
  ...
  def parse(self, response):
    for i in range(len(first_names)):
      ...
      yield SplashRequest(results_link, callback=self.parse_results, meta={'splash' : {'endpoint' : 'render.html', 'args' : { 'wait' : 5.0 }}})
  ...
  def parse_results(self,response):
    ...
    for j in range(number_of_pages):
      ...
      yield SplashRequest(candidate_link, callback=self.parse_the_candidate, meta={'splash' : {'endpoint' : 'render.html', 'args' : { 'wait' : 6.0 }}})
  ...
  def parse_the_candidate(self, response):
    ...
    for link in candidate_links:
      link = "https:"+ link
      yield SplashRequest(link, callback=self.parse_the_article, meta={'splash' : {'endpoint' : 'render.html', 'args' : { 'wait' : 3.0 }}, 'article_link': link})
